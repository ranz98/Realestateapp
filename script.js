document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Helper: format price short (M for millions, K for thousands) ---
    ...
    /* ════════════════════════════════════════════════════════════════
       ...
       Snaps:  10 / 30 / 50 / 70 / 90 (vh = listings sheet height %)
       Drag down past 8% → full MAP. Drag up past 92% → full LIST.
       ════════════════════════════════════════════════════════════════ */
    (function setupSplitGrabber() {
        ...
        function setSplitPct(pct, persist) {
            ...
            if (map) setTimeout(() => { try { map.invalidateSize(); } catch (_) {} }, 30);
        }
        ...
        // Wrap applyMobileMode to restore last split position when entering split mode.
        const _origApply = applyMobileMode;
        applyMobileMode = function (mode) {
            _origApply(mode);
            if (mode === 'split') {
                const saved = parseFloat(localStorage.getItem(STORAGE_KEY) || '60');
                setSplitPct(isFinite(saved) ? saved : 60, false);
            }
        };

        // If we're already in split mode at boot (restored from localStorage), seed the variable now.
        if (mainContainer.classList.contains('split-mode')) {
            const saved = parseFloat(localStorage.getItem(STORAGE_KEY) || '60');
            setSplitPct(isFinite(saved) ? saved : 60, false);
        }

        let dragging = false, startY = 0, startPct = 60, vh = window.innerHeight;
        ...
        function onStart(clientY) {
            ...
            startY = clientY;
            startPct = parseFloat(getComputedStyle(mainContainer).getPropertyValue('--split-pct')) || 60;
        ...
        function onMove(clientY) {
            if (!dragging) return;
            // Sheet is anchored bottom. Dragging DOWN shrinks the sheet (map grows).
        ...
        function onEnd() {
            ...
            const cur = parseFloat(getComputedStyle(mainContainer).getPropertyValue('--split-pct')) || 60;
            if (cur <= 8)  { applyMobileMode('map');  return; }
            if (cur >= 92) { applyMobileMode('list'); return; }
    ...
    /* ── Card slider helpers (Native Scroll Modernized) ── */
    function _updateCardSlider(wrapper) {
        const s = parseInt(wrapper.dataset.slide) || 0;
        const w = wrapper.offsetWidth;
        if (w > 0) {
            wrapper.scrollTo({ left: s * w, behavior: 'smooth' });
        }
    ...
    /* ── Native Scroll Interaction Handlers ── */
    let _moved = false;

    if (grid) {
        // 1. Update dots and slide index during native swipe
        ...
        grid.addEventListener('touchend', e => {
            ...
            _moved = _touchMoved;
            setTimeout(() => { _moved = false; _touchMoved = false; }, 50);
        });


        // Mouse Drag-to-Scroll Support
        let _isMouseDown = false;
        let _scrollStartX, _scrollLeftInitial;
        ...
        grid.addEventListener('mousedown', (e) => {
            ...
            wrapper.style.scrollSnapType = 'none'; // Disable snapping during manual drag
        ...
        window.addEventListener('mousemove', (e) => {
            ...
            if (dx > 5 || dy > 5) _isScrolling = true;

            const wrapper = document.querySelector('.card-slider-wrapper:hover') ||
                Array.from(document.querySelectorAll('.card-slider-wrapper')).find(w => w.contains(e.target));
            if (!wrapper) return;

            const x = e.pageX - wrapper.offsetLeft;
            const walk = (x - _scrollStartX) * 1.5;
            wrapper.scrollLeft = _scrollLeftInitial - walk;
        });

        window.addEventListener('mouseup', () => {
            if (!_isMouseDown) return;
            _isMouseDown = false;
            const wrappers = document.querySelectorAll('.card-slider-wrapper');
            wrappers.forEach(w => {
